...
| **Mobile App** | Flutter (Dart), Provider, YouTube Data API |
| **UI Components** | ShadCN UI (Web), Custom Glassmorphic UI (Mobile) |
| **Backend** | Firebase (Auth/Users), Supabase (MCQs/Courses) |
| **Database** | Firestore & PostgreSQL (via Supabase) |
| **Media/Storage** | Supabase Storage (CORS optimized) |
| **AI Integration** | OpenRouter API (Gemini/DeepSeek) |
...
**Passion Academia** utilizes a sophisticated hybrid backend to ensure maximum performance and cross-platform compatibility:

- **Firebase (Firestore + Auth)**: Primary source of truth for user accounts, roles, and authentication. This ensures the Mobile App and Web App share the same user database.
- **Supabase (PostgreSQL + Storage)**: Handles the heavy lifting for academic content. MCQs, Courses, and Chapters are fetched from Supabase, and profile pictures are hosted here to avoid Firebase CORS limitations.
- **YouTube Data API**: Specifically integrated into the mobile app to provide smart video recommendations based on the active lesson.

---

## 📱 Mobile App Features

The mobile version (built with Flutter) offers a premium, app-centric experience:

- **Infinity Loader**: A custom-designed, sleek loading animation used throughout the application.
- **AI Daily Quizzes**: Practice with fresh questions generated by AI every day.
- **Smart Recommendations**: Automatic YouTube video suggestions for every chapter.
- **Admin Console**: A professional tabbed dashboard for managing users and courses on the go.
- **Gamification**: Streak tracking and XP system to keep students engaged.
...
- [x] Video lecture integration (YouTube API)
- [x] Hybrid Backend (Firebase + Supabase)
- [ ] Live chat support
- [ ] Gamification features (Streaks/XP)
